...
def load_metadata(name:str, checkpoint_path, new_run, percentiles:Tuple[Float]):
    if f"{name}_{percentiles[-1]}_{hash_dict(percentiles)}.json" in os.listdir(checkpoint_path) and not new_run:
        print(f"Metadata Found")
        with open(os.path.join(checkpoint_path, f"{name}_{percentiles[-1]}_{hash_dict(percentiles)}.json"), "r") as f:
            metadata = json.load(f)
    else:
        metadata = {"last_epoch":0}
    return metadata

def get_model(name:str, config_file, checkpoint_path, logger, new_run, percentiles:Tuple[Float])-> Tuple[nn.Module, Any]:
    with open(config_file, "r") as f:
        config = json.load(f)
    groups = len(percentiles)
    if groups > 1:
        for i in config:
            i["group"] = len(percentiles)
    ...
    if f"{name}_{percentiles[-1]}_{hash_dict(percentiles)}.safetensors" in os.listdir(checkpoint_path) and not new_run:
        print(f"Model Found")
        model_path = os.path.join(checkpoint_path, f"{name}_{percentiles[-1]}_{hash_dict(percentiles)}.safetensors")
        opt_path = os.path.join(checkpoint_path, f"{name}_{percentiles[-1]}_{hash_dict(percentiles)}_ADAM.pt")
        model_weights = st.load_file(model_path)
        model_base.load_state_dict(model_weights)
...
def save_model(model:nn.Module, opt:torch.optim.Optimizer, model_type, checkpoint_path, model_config, percentiles:Tuple[Float], metadata=None):
    ...
    model_path = os.path.join(checkpoint_path, f"{model_type}_{percentiles[-1]}_{hash_dict(percentiles)}.safetensors")
    opt_path = os.path.join(checkpoint_path, f"{model_type}_{percentiles[-1]}_{hash_dict(percentiles)}_ADAM.pt")
    with open(os.path.join(checkpoint_path, f"{model_type}_{percentiles[-1]}_{hash_dict(percentiles)}.json"), "w") as f:
...
def get_mask(x, segment_model, classes:Tuple[Float]):
    with torch.no_grad():
        out = mask_from_classes(segment_model(x), classes)
        return [out == i for i in range(1, len(classes) + 1)]

    
def main(data_path, model_type, model_config, checkpoint_path, log_file, new_run, seg_model_name, seg_model_config, percentiles:Tuple[Float]):
    ...
    model, opt_state = get_model(model_type, model_config, checkpoint_path, logger, new_run, percentiles)
    ...
    for e in range(metadata["last_epoch"], EPOCHS):
        ...
        with tqdm(total=len(train_dataloader)*local_batch_size,desc=f"Epoch {e+1} Training Loss: NaN") as pbar:
            for coarse, dif in train_dataloader:
                ...
                with torch.cuda.stream(mask_stream):
                    mask = get_mask(coarse, seg_model, percentiles)
                with torch.cuda.stream(model_stream):
                    pred = model.forward(coarse.repeat(1, len(percentiles), 1, 1))
        ...
        save_model(model, opt, model_type, checkpoint_path, model_config, percentiles, metadata)
...
if __name__ == "__main__":
    ...
    ap.add_argument("--checkpoint_path", default="/content/drive/MyDrive/checkpoints/tuning_v2", help="path to model config")
    ap.add_argument("--log_file", default="/content/drive/MyDrive/logs/tuning_v2.log", help="path to log file")
    ...
    ap.add_argument("--percentiles", type=float, nargs="+", default=[0.5], help="list of percentiles")
