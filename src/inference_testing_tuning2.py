...
def get_model(name:str, config_file, checkpoint_path, logger, percentiles)-> Tuple[nn.Module, dict]:
    ...
    if f"{name}_{percentiles[-1]}_{hash_dict(percentiles)}.safetensors" in os.listdir(checkpoint_path):
        print("Model Found")
...
def main(model_type, model_config, checkpoint_path, log_file, no_segment, seg_model_name, seg_model_config, percentiles):
    # ---------- Simulation Setup ---------------
    high_res = 1024
    low_res = 64
    density = 1.0
    ...
    #-----------ML setup------------------
    model = get_model(model_type, model_config, checkpoint_path, logger, percentiles)
    model.to(device)
    if not no_segment:
    ...
    with tqdm(total=int(1/coarse_dt),desc=f"Control Error: NaN, LC Error: NaN") as pbar:
        for i in range(int(1/coarse_dt)):
            ...
            with torch.cuda.stream(model_stream):
                ...
                delta_v = model.forward(coarse_norm.repeat(1, len(percentiles), 1, 1)) #fix grouping (1, group-1, 1, 1)

            with torch.cuda.stream(mask_stream):
                torch.cuda.current_stream().wait_stream(LC_stream)
                mask = get_mask(coarse_norm, seg_model, percentiles)
...
if __name__ == "__main__":
    ...
    ap.add_argument("--checkpoint_path", default="/content/drive/MyDrive/checkpoints/tuning_v2", help="path to model checkpoints")
    ap.add_argument("--log_file", default="/content/drive/MyDrive/logs/inference_tuning_v2.log", help="path to log file")
