...
from .tools import paramToList, structureLoader, getAct, getPool
...
class CNN(nn.Module):
    ...
    Args:
        config (dict): A dictionary containing hyperparameters:\n
            ...
            pooling (dict, optional): Pooling parameters: (\n
    ...
    def forward(self, x):
        ...
        return self.pool(layer(x))



class PreluCNN(nn.Module):
    """
    Convolution layers with customisable hyperparameters.
    Args:
        config (dict): A dictionary containing hyperparameters:\n
    ...
    def __init__(self, config):
        ...
        try:
            if config["bn"]:
                self.layers = nn.ModuleList([nn.Sequential(nn.Conv2d(structure[i], structure[i+1], kernel_size=kernel_sizes[i], stride=strides[i], padding=paddings[i], groups=group[i]), nn.BatchNorm2d(structure[i+1]), nn.PReLU(structure[i+1])) for i in range(len(structure)-1)])
            else:
                self.layers = nn.ModuleList([nn.Sequential(nn.Conv2d(structure[i], structure[i+1], kernel_size=kernel_sizes[i], stride=strides[i], padding=paddings[i], groups=group[i]), nn.PReLU(structure[i+1])) for i in range(len(structure)-1)])
        except(KeyError):
            self.layers = nn.ModuleList([nn.Sequential(nn.Conv2d(structure[i], structure[i+1], kernel_size=kernel_sizes[i], stride=strides[i], padding=paddings[i], groups=group[i]), nn.PReLU(structure[i+1])) for i in range(len(structure)-1)])
        try:
            self.pool = getPool(config["pooling"])
    ...
    def forward(self, x):
        for i, layer in enumerate(self.layers):
            if i < len(self.layers) - 1:
                x = F.dropout(layer(x), p=self.dropouts[i], training=self.training)
        return self.pool(layer(x))
